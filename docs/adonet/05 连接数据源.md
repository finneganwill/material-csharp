...
### 02 连接字符串生成器
在 ADO.NET 的早期版本中，不会对具有串联字符串值的连接字符串进行编译时检查，因此在运行时会产生 ArgumentException。 并且，不同的 Data Provider 支持的连接字符串关键字的语法不同，这使得手动构造有效连接字符串变得很困难。 为解决这个问题，ADO.NET 2.0 为每个 .NET Framework 数据提供程序引入了**连接字符串生成器**，每个 Data Provider 包括一个从 `DbConnectionStringBuilder` 继承的强类型连接字符串生成器类。 
