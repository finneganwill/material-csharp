...
## 九、队列
队列（Queue）是一种具有先进先出的数据结构。
#### .NET 类
`Queue<T>` 是 .NET System.Colloections.Generic 中提供的存储相同指定类型实例的可变大小、后进先出（LIFO）集合，它替代了早期在 System.Collections 中提供的 `Queue` 类。当你需要临时存储信息时，堆栈和队列很有用。也就是说，您可能希望在检索元素值后丢弃元素。
