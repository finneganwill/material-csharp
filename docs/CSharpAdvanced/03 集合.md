...
栈是一种具有先进后出的数据结构。
#### .NET 类
`Stack<T>` 是 .NET System.Colloections.Generic 中提供的存储相同指定类型实例的可变大小、后进先出（LIFO）集合，它替代了早期在 System.Collections 中提供的 `Stack` 类。当你需要临时存储信息时，堆栈和队列很有用。也就是说，您可能希望在检索元素值后丢弃元素。

>如果需要以相反的顺序访问信息，则使用 Stack；如果需要按照信息在集合中存储的顺序访问信息，则使用Queue。
...
!!! Note
	Stack<T\> 具有一下特点：<br>
	:one: 先进后出。最先被添加的元素被存储在栈底，先添加的元素后被取出。<br>
	:two: 元素可重复。允许添加重复元素。<br>
	:three: 元素可为 null 。 接受null作为引用类型的有效值。<br>
	:four: 非线程安全。该类不是线程安全的，当需要从多个线程并发访问集合时，ConcurrentStack<T\> 类型
