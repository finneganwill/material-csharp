...
5. 集合的每个元素都是可作为DictionaryEntry 对象访问的键/值对。
...
#### 继承体系
```csharp
public class SortedList<TKey,TValue> : System.Collections.Generic.ICollection<System.Collections.Generic.KeyValuePair<TKey,TValue>>, 
...
#### 操作
[SortedList<TKey,TValue>](https://learn.microsoft.com/zh-cn/dotnet/api/system.collections.generic.sortedlist-2?view=net-7.0)只要集合未修改，就可以同时支持多个读取器。 即便如此，通过集合枚举本质上也不是线程安全的过程。 若要确保枚举过程中的线程安全性，可以在整个枚举过程中锁定集合。 若要允许多个线程访问集合以进行读写操作，则必须实现自己的同步。

[](https://learn.microsoft.com/zh-cn/dotnet/api/system.collections.generic.sortedlist-2?view=net-7.0#see-also)
