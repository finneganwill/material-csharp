...
HashSet 类提供了高性能的集合操作，如交集、并集、差集等。集合是不包含重复元素的集合，其元素没有特定的顺序。

HashSet 类基于数学集模型，提供了类似于访问 Dictionary 或 Hashtable 集合的键的高性能集操作。简单来说，HashSet 类可以看作是一个没有值的 Dictionary 集合。
